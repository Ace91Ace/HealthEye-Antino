...
""");
  ...
  Future<String> analyzeFoodImage({
    ...
    if (kIsWeb) {
      // For web, we need to use a different approach since File operations aren't fully supported
      // The imageFile on web is likely coming from XFile, so we can use its readAsBytes method
      // This assumes the image picker is returning an XFile or similar object that works on web
      try {
        // Try to get the bytes from the HTML file
        ...
        _isLoading = false;
        setState(() {});
        return 'Error: Unable to process image on web. Please try again or use a different image.';
      }
    } else {
      // For mobile platforms, we can use the standard File API
...
""");
  ...
  Future<void> captureImage({
    ...
    if (image != null) {
      if (kIsWeb) {
        // On web, we can't use File directly with the image path
        // Instead, we'll store the XFile object directly
        if (isFrontImage) {
          _frontImage = image as dynamic;
        } else {
          _nutritionLabelImage = image as dynamic;
        }
      } else {
        // On mobile platforms, we can use File with the image path
        if (isFrontImage) {
  ...
  ImageProvider getImageProvider(dynamic imageSource) {
    if (imageSource == null) {
      debugPrint('Image source is null, returning placeholder');
      return _createPlaceholderImage();
    }
    
    if (kIsWeb) {
      // For web, if it's an XFile, we need to handle it differently
      if (imageSource is dynamic && imageSource.runtimeType.toString().contains('XFile')) {
        try {
          debugPrint('Processing XFile on web platform - returning placeholder');
          // For web XFile objects, it's better to use the FutureBuilder approach in the UI
          // Here we'll just return a placeholder
          return _createPlaceholderImage();
        } catch (e) {
          debugPrint('Error creating image provider for web: $e');
          return _createPlaceholderImage();
        }
      } else {
        debugPrint('Unsupported image source type on web: ${imageSource.runtimeType}');
        return _createPlaceholderImage();
      }
    } else {
      // For mobile, use FileImage
      if (imageSource is File) {
        debugPrint('Using FileImage for mobile platform');
        return FileImage(imageSource);
      } else {
        debugPrint('Unsupported image source type on mobile: ${imageSource.runtimeType}');
        return _createPlaceholderImage();
      }
    }
  }
